...
Deployment Protection is scoped by domain, never by path. There's no way to leave
...
1. **Send `x-vercel-protection-bypass`** (Protection Bypass for Automation). Works, but it's a
   second secret every client has to hold, and not every client can set custom headers, so it
   limits which tools can reach you.
...
The failure looks like a routing bug rather than an auth one. You get an HTML login page or a
404 where you expected JSON, so the instinct is to check your route. Check the deployment
protection setting first, because the request never reached your code.
